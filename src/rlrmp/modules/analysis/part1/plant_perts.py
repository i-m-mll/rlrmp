...
def setup_eval_tasks_and_models(task_base, models_base, hps):
    """Set up tasks and models for plant perturbation analysis.

    In the eager-models architecture:
    1. Insert intervention components into models via graph surgery
    2. Schedule intervention params on tasks
    """
    try:
        pert_params_fn = PLANT_PERT_FNS[hps.pert.type](hps)
    except KeyError:
    ...
    models = add_plant_intervention_to_ensemble(
        ...
        active=False,  # Will be controlled by task params
    )
    ...
    all_tasks, all_models = jtree.unzip(
        jt.map(
            lambda pert_amp: schedule_intervenor(
                ...
                intervenor_params=pert_params_fn(pert_amp),
