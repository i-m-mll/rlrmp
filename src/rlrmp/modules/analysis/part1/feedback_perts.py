...
components_labels = dict(xy=COORD_NAMES, aligned=(r"\parallel", r"\bot"))
...
def _setup_rand(task_base, models_base, hps):
    ...
    impulse_directions = jt.map(
        lambda task: task.validation_trials.intervene[FB_INTERVENOR_LABEL].arrays[
            :, hps.pert.start_step
        ],
        all_tasks,
        is_leaf=is_module,
...
def _setup_xy(task_base, models_base, hps):
    ...
    feedback_var_idxs = LDict.of("pert__var")(dict(zip(PERT_VAR_NAMES, range(len(PERT_VAR_NAMES)))))
    coord_idxs = dict(zip(COORD_NAMES, range(len(COORD_NAMES))))

    impulse_xy_conditions = LDict.of("pert__var").fromkeys(
        PERT_VAR_NAMES, dict.fromkeys(COORD_NAMES)
    )
    impulse_xy_conditions_keys = jtree.key_tuples(
        impulse_xy_conditions,
        keys_to_strs=True,
        is_leaf=is_none,
    )
...
ANALYSES = {
    "plot--measures": Violins(
        inputs=Violins.Ports(input="measures").map_figs_at_level(
            "measure",
