...
class ScienceFragment:
    """One capability or objective contribution to emitted science metadata."""

    mode: ScienceMode | None = None
    loss_spec: dict[str, Any] | None = None
    fidelity_status: dict[str, Any] | None = None


@dataclass(frozen=True)
class LoweredScience:
    """Merged training-science metadata plus lowerer provenance."""
...
def _mode_fragment(field: str, mode: ScienceMode):
    def lower(hps: TreeNamespace) -> ScienceFragment | None:
        return ScienceFragment(mode=mode) if _enabled(hps, field) else None

    return lower


def _target_relative(hps: TreeNamespace) -> ScienceFragment | None:
    if not _enabled(hps, "target_relative_multitarget"):
        return None
    mode = (
    ...
    return ScienceFragment(mode=mode)
...
def _time_indexing(hps: TreeNamespace) -> tuple[dict[str, Any], str]:
    ...
    tail_mode = str(hps.loss.delayed_movement_cost_tail_mode)
    return (
        {
            ...
            "canonical_movement_horizon_steps": CS_STAGE_COUNT,
            "cost_tail_mode": tail_mode,
            "post_horizon_tail": (
                "zero_weight_after_canonical_horizon"
                if tail_mode != DELAYED_MOVEMENT_COST_TAIL_FLAT_AFTER_HORIZON
...
def _common_loss(hps: TreeNamespace, objective: str) -> dict[str, Any]:
    time_indexing, fact_t = _time_indexing(hps)
    return {
...
def _full_qrf_loss(hps: TreeNamespace) -> dict[str, Any]:
    ...
    time_indexing, _fact_t = _time_indexing(hps)
    ...
    return {
        ...
        "grouped_reduction_implementation": (
            "rlrmp_bridge_pending_feedbax_69d8d76" if normalization["enabled"] else "not_enabled"
        ),
        "source_module": (
...
def _fidelity(
    ...
    *,
    ...
    backend = str(getattr(hps.model, "plant_backend", CS_LSS_PLANT_BACKEND))
    exact_lss = backend == CS_LSS_PLANT_BACKEND
    extra_terms = (
        []
    ...
    payload = {
        ...
        "exact_objective_terms": exact_lss if exact_from_backend else False,
        "exact_objective_terms_scope": exact_scope,
        "objective_fidelity": {
            "implemented_terms": list(implemented),
            "omitted_terms": ([] if exact_lss else omitted) if exact_from_backend else omitted,
...
_PARTIAL_FIDELITY = {
    ...
    "exact_scope": "false because force/filter-state and disturbance-integrator state costs from the analytical C&S schedule are omitted from the current Feedbax GRU loss contract",
    "implemented": ("running_position_cs_eq15_power6", "terminal_position", "running_velocity_cs_eq15_power6", "terminal_velocity", "command_quadratic_nn_output"),
    "omitted": [
        {"term": "force_filter_state_cost", "analytical_role": "unit-weight force/filter state cost in the C&S 8D schedule", "status": "not_synthesized_in_feedbax_gru_loss"},
        {"term": "disturbance_integrator_state_cost", "analytical_role": "unit-weight disturbance-integrator state cost in the C&S 8D schedule", "status": "not_synthesized_in_feedbax_gru_loss"},
    ],
...
_PARTIAL_FORCE_FIDELITY = {
    ...
    "exact_scope": "ablation only: old partial position/velocity terms are kept, control is moved to intended net.output, and running force/filter state cost is added; this is not the full Q/R/Q_f objective",
    "implemented": ("running_position_cs_eq15_power6", "terminal_position", "running_velocity_cs_eq15_power6", "terminal_velocity", "intended_command_quadratic_net_output", "running_force_filter_state_cost"),
    "omitted": [
        {"term": "disturbance_integrator_state_cost", "analytical_role": "unit-weight disturbance-integrator state cost in the C&S 8D schedule", "status": "intentionally_omitted_for_force_filter_ablation"},
        {"term": "terminal_force_filter_and_integrator_Q_f", "analytical_role": "terminal full-state Q_f costs", "status": "not_synthesized_in_partial_ablation"},
    ],
    "selection_policy": "rollout validation loss only; analytical action and I/O metrics are audit-only",
}
_FULL_QRF_FIDELITY = {
    "objective": CS_FULL_ANALYTICAL_QRF_LOSS_OBJECTIVE,
    "exact_scope": "true for the implemented training scalar when plant_backend='cs_lss': the loss evaluates canonical C&S delay-augmented Q_t, R_t, and Q_f on the exposed LinearStateSpace state and command history",
    "implemented": ("delay_augmented_state_running_Q_t", "command_running_R_t", "delay_augmented_terminal_Q_f"),
    "omitted": ["cs_lss_state_unavailable"],
    "selection_policy": "rollout validation loss uses the same full analytical Q/R/Q_f training scalar; analytical action and I/O metrics remain audit-only",
...
def _objective_fragment(objective: str, loss_builder, fidelity_builder):
    def lower(hps: TreeNamespace) -> ScienceFragment | None:
        if str(hps.loss.objective) != objective:
            return None
        return ScienceFragment(
            loss_spec=loss_builder(hps),
            fidelity_status=fidelity_builder(hps),
        )

    return lower


SCIENCE_LOWERERS = OrderedLowererRegistry[
    TreeNamespace, ScienceFragment
](
    [
        LowererRegistration("delayed_reach", 0, "rlrmp.train.delayed_reach", _mode_fragment("delayed_reach", ScienceMode.DELAYED_REACH)),
        LowererRegistration("target_relative", 10, "rlrmp.train.target_relative", _target_relative),
        LowererRegistration("broad_epsilon", 20, "rlrmp.train.broad_epsilon_training", _mode_fragment("broad_epsilon_training", ScienceMode.BROAD_EPSILON)),
        LowererRegistration("broad_epsilon_pgd", 21, "rlrmp.train.broad_epsilon_training", _mode_fragment("broad_epsilon_pgd_training", ScienceMode.BROAD_EPSILON_PGD)),
        LowererRegistration("policy_adversary", 22, "rlrmp.train.policy_adversary_native", _mode_fragment("policy_adversary_training", ScienceMode.POLICY_ADVERSARY)),
        LowererRegistration("perturbation", 30, "rlrmp.train.fixed_target_perturbation_training", _mode_fragment("perturbation_training", ScienceMode.PERTURBATION)),
        LowererRegistration("objective.full_qrf", 100, "rlrmp.loss.full_analytical_qrf", _objective_fragment(CS_FULL_ANALYTICAL_QRF_LOSS_OBJECTIVE, _full_qrf_loss, partial(_fidelity, **_FULL_QRF_FIDELITY))),
        LowererRegistration("objective.partial_force_filter", 100, "rlrmp.loss.partial_net_force_filter", _objective_fragment(CS_PARTIAL_NET_FORCE_FILTER_LOSS_OBJECTIVE, _partial_force_filter_loss, partial(_fidelity, **_PARTIAL_FORCE_FIDELITY))),
        LowererRegistration("objective.partial", 100, "rlrmp.loss.partial_feedbax", _objective_fragment(CS_PARTIAL_FEEDBAX_LOSS_OBJECTIVE, _partial_loss, partial(_fidelity, **_PARTIAL_FIDELITY))),
    ]
)


def lower_training_science(hps: TreeNamespace) -> LoweredScience:
    """Compose registered capability and objective fragments for one authored row."""

    contributions = SCIENCE_LOWERERS.lower(hps)
    modes = [item.fragment.mode.value for item in contributions if item.fragment.mode is not None]
    losses = [item.fragment.loss_spec for item in contributions if item.fragment.loss_spec is not None]
    fidelities = [
        item.fragment.fidelity_status
        for item in contributions
        if item.fragment.fidelity_status is not None
    ]
    if len(losses) != 1 or len(fidelities) != 1:
        raise ValueError(
            "training science must lower exactly one loss and fidelity fragment; "
            f"found losses={len(losses)}, fidelities={len(fidelities)}"
        )
    return LoweredScience(
        training_mode="+".join(modes) if modes else ScienceMode.NOMINAL.value,
        loss_spec=losses[0],
        fidelity_status=fidelities[0],
        lowerer_ids=tuple(item.lowerer_id for item in contributions),
    )
