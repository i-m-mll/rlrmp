...
class ClosedLoopDistillationLoss(AbstractLoss):
    ...
    The loss is called by Feedbax after the normal ``TaskTrainer`` closed-loop
...
def smoke_train_command(*, spec_path: Path = DEFAULT_SPEC_PATH) -> list[str]:
    """Return the local CPU smoke command that exercises TaskTrainer/train_pair."""
...
def build_closed_loop_distillation_spec(args: argparse.Namespace) -> dict[str, Any]:
    ...
    spec = {
        ...
        "closed_loop_semantics": {
            ...
            "trainer_contract": (
                "Use the existing Feedbax TaskTrainer/run_component closed-loop rollout; "
                "student actions are generated by the standard h0 graph and feed future "
                "plant state/observations through the normal task surface."
            ),
        },
        "base_contract": {
            "issue": BASE_ISSUE_ID,
...
def build_closed_loop_trainer(spec: dict[str, Any], *, n_batches: int | None = None) -> TaskTrainer:
    """Build the normal Feedbax trainer used by smoke and approved full runs."""

    student = spec["student_contract"]
    total_batches = int(n_batches or student["n_train_batches"])
...
def run_closed_loop_distillation_training(
    *,
    ...
    smoke: bool = False,
    setup_pair_fn=None,
    trainer_factory=build_closed_loop_trainer,
    train_pair_fn=train_pair,
    loss_factory=build_closed_loop_loss,
) -> dict[str, Any]:
    """Run smoke or approved full training through Feedbax ``train_pair``."""

    validate_run_spec(spec)
    if not smoke and not confirm_full_train:
        raise FullTrainingApprovalRequiredError(
            "Full closed-loop extLQG distillation is wired through Feedbax "
            "TaskTrainer/train_pair but requires explicit user launch approval. "
