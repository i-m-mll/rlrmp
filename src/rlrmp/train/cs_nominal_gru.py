...
def _update_adaptive_epsilon_state(
    ...
    *,
    ...
    damage_for_math = ratio_eps if damage_ema is None else max(float(damage_ema), ratio_eps)
    current_log_damage_ema = (
        math.log(damage_for_math) if damage_ema is not None and damage_for_math > 0.0 else None
    )
    ...
    log_ratio_error = (
        math.log(damage_for_math / max(target, ratio_eps)) if target > 0.0 else 0.0
    )
