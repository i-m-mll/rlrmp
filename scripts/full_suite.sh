...
# recording a false green (or false red) full-suite memo against the wrong
# tree.
...
' "$EXPECTED_RLRMP_SRC" 2>&1)"; then
    echo "ERROR: scripts/full_suite.sh refused to run: stale editable install detected." >&2
...
exec uv run --no-sync python "$REPO_ROOT/scripts/full_suite.py" "$@"
