...
`rlrmp.report.bridge_certificate_notes` owns the standard-certificate table,
mode-aware component presentation, annotations, and failure-decomposition
companion. Its rendered output is the presentation authority; do not recreate
those rules in result-local scripts or prose.

When presenting standard-certificate results in conversation, show the rendered
table rather than only a prose summary. Recompute available certificate inputs
instead of leaving components partial when possible. Keep evaluation lenses
(for example nominal-clean, Riccati-epsilon, process-noise, coverage-induced,
and held-out validation) separate from training axes such as nominal versus
robust or coverage versus no coverage.
