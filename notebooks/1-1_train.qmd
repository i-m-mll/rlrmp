...
PARAMS_DIR = Path('../parameters')

for d in (FIGS_DIR, MODELS_DIR, PARAMS_DIR):
    d.mkdir(parents=True, exist_ok=True)
```

```{python}
models_filename = "1-1_trained_models.eqx"
...
def savefig(fig, label, ext='.svg', transparent=True, **kwargs): 
    fig.savefig(
        FIGS_DIR / f"{label}{ext}",
        transparent=transparent, 
        **kwargs, 
    )
    
def saveplotly(fig, label):
    fig.write_html(FIGS_DIR / f'{label}.html')
    fig.write_json(FIGS_DIR / f'{label}.json')
...
feedback_noise_std = 0.025
motor_noise_std = 0.025
...
```{python}
disturbance_levels = [0, 0.1, 0.25]

def curl_field(curl_std):
    return CurlField.with_params(
        amplitude=lambda trial_spec, *, key: curl_std * jr.normal(key, (1,)),
    )
...
for curl_std, (task_, models_) in tqdm(list(zip(disturbance_levels, task_model_pairs))):
    trained_models[curl_std], train_histories[curl_std] = trainer(
        task_, 
        models_,
        ensembled=True,
        n_batches=n_batches, 
...
Since we haven't done this in any later notebooks yet, I'm including it here for reference.
...
```{python}
for curl_std, history in train_histories.items():
    label = f"1-1_loss-history_curl-std-{curl_std}"
    
    p1, _ = loss_history(history)
    p2, _ = loss_mean_history(history)
    p3 = fbp.loss_history(history)
    
    savefig(p1, label + '_replicates')
    savefig(p2, label)
    saveplotly(p3, label)
```
