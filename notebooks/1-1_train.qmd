...
## Keep best parameters for each model replicate

**TODO**: Ensure the final parameters get saved along with the rest of the `train_histories`, since we may be discarding them otherwise, here.
**TODO**: Also, `best_iteration_by_replicate` and `best_replicates` are not hyperparameters, but are currently saved with the hyperparameters; they should probably be saved as part of `train_histories`
...
## Determine models to be excluded from analysis, based on loss quantile bounds

```{python}
fig_subdir = "best-loss-distributions-by-replicate"
```
...
**TODO**: the losses are not terribly variable, so it might be better to exclude models based on a behavioural measure, such as the endpoint error on the control (no perturbation) task
...
To reload our models in later notebooks, we will need a function that regenerates a skeleton PyTree with the same structure as `saved_models`, given the stored `hyperparameters`. See the function `setup_models` in `src/rnns_learn_robust_motor_policies/part1_setup.py`.

### Save training histories

In later analyses we may be interested in the history of the losses or model parameters across training. These should also be saved to disk. 
...
model_parameter_histories_ = load(
    str(MODELS_DIR / train_histories_basename) + ".eqx",
    partial(setup_train_histories, task_train, trained_models),
)
```
