...
disturbance_type = 'curl'  # or 'random'
hidden_size  = 50
...
```{python}
suffix = f"{disturbance_type}_noise-{feedback_noise_std}-{motor_noise_std}_delay-{feedback_delay_steps}"
```
...
if disturbance_type == 'curl':
    disturbance_stds = jnp.linspace(0, 1, 5).tolist()
    ...
    disturbance_stds = jnp.linspace(0, 0.1, 2).tolist()
...
for curl_std, (task_, models_) in tqdm(list(zip(disturbance_stds, task_model_pairs))):
