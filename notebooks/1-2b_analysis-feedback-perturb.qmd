...
disturbance_train_stds_lohi = [disturbance_train_stds[0], disturbance_train_stds[-1]]

trained_models = OrderedDict(zip(disturbance_train_stds, trained_models_load))
```
...
model_parameter_histories = dict(zip(disturbance_train_stds, model_parameter_histories))
...
impulse_amplitudes_lohi = {k: (v[0], v[-1]) for k, v in impulse_amplitudes.items()}
...
def evaluate_all_impulse_responses():
    """Wrapped as a function for the convenience of estimating the amount of memory needed for the result"""
    return {
        analysis_variant: {
...
plot_states = jt.map(where_plot, plot_states, is_leaf=is_module)
```
...
for path, fig in tqdm(figs_flatten_with_paths(figs)):
    ...
    if disturbance_train_std in disturbance_train_stds_lohi and pert_var == 'vel':
        fig.show()
```
...
def get_all_profiles(all_plot, var_label, disturbance_stds=None, colors=None):    
    ...
    for path, fig in figs_flatten_with_paths(figs):
        ...
        pert_var = path[1].key
        pert_condition = '-'.join(dict_key.key for dict_key in path[1:-1])
        pert_amp = path[-1].key
...
for path, fig in tqdm(figs_flatten_with_paths(figs)):
    ...
    pert_var = path[2].key
    pert_condition = '-'.join(dict_key.key for dict_key in path[2:-1])
...
def get_summary_profiles(var, var_label):
    figs = get_all_profiles(var, var_label, disturbance_train_stds_lohi)
    ...
    figs_t = {
        coord_name: jt.transpose(
            ...
            jt.structure(OrderedDict.fromkeys(pert_var_names, '*')), 
            None, 
            OrderedDict(figs_by_pert_var),
        )
...
for path, fig in tqdm(figs_flatten_with_paths(figs)):
    ...
    if impulse_amplitude['pos'] in impulse_amplitudes_lohi['pos']:
        fig.show()
...
all_measures_lohi = {
    measure_name: jt.map(
        lambda train_condn_measures: subdict(train_condn_measures, disturbance_train_stds_lohi),
        measure,
        is_leaf=is_type(OrderedDict),
    )
...
measure_ranges = {
    measure_name: (
            jnp.min(measure_data_stacked),
            jnp.max(measure_data_stacked),   
    )
...
def get_violins_comparing_amplitudes(measure_data, measure_name, pert_var, annotation_kws=None, layout_kws=None):
    ...
    # Impulse perturbation amplitude (x axis)
    for i, pert_amp in enumerate(impulse_amplitudes[pert_var]):
        # Disturbance train std. (split)
        for j, disturbance_train_std in enumerate(disturbance_train_stds_lohi):
            fig.add_trace(
                go.Violin(
                    x=np.full((n_dist,), pert_amp),
                    y=measure_data[i][pert_var][disturbance_train_std].flatten(),
    ...
    fig.update_layout(
        ...
        # xaxis_tickvals=list(measure_data.keys()),
        # yaxis_range=[0, None],
        legend_title="Disturbance train std.",
        violinmode='overlay',
        violingap=0,
        **layout_kws,
    )
