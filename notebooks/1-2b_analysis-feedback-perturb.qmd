...
## Choose a task variant for plotting

```{python}
from feedbax import tree_labels

analysis_variant: Literal['xy', 'rand'] = 'xy'
...
### Control forces in x/y directions
...
figs = {
    ...
    for imp_condn, controls in all_controls.items()
}
```

```{python}
n = np.prod(jt.leaves(all_controls_imp)[0].shape[:-2])
...
### Control forces aligned to perturbation direction

```{python}
def project_and_lump(all_states, where, directions):
    # The tree structure of `directions` matches `all_states`. 
    # So we can re-orient the variable located at `where`,
    # and then lump together all the arrays below the first level of `all_states`, 
    # which we assume to be the "impulse variable" level (i.e. pos or vel).
    all_var = jt.map(where, all_states, is_leaf=is_module)
...
```{python}
n = np.prod(jt.leaves(all_controls)[0].shape[:-2])
...
### Velocity profiles in x/y directions
...
figs = {
    imp_condn: {
        ...
        for label, i in dict(v_x=0, v_y=1).items()
    }
    for imp_condn, vels in all_vels.items()
}
```

```{python}
n = np.prod(jt.leaves(all_vels)[0].shape[:-2])
...
### Velocity profiles aligned to perturbation direction

```{python}
all_vels = project_and_lump(
    all_states, 
    lambda states: states.mechanics.effector.vel,
    directions,
)
