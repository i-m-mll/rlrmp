...
trained_models = dict(zip(disturbance_stds_train, trained_models))
```
...
for pert_condition in figs:
    for disturbance_std_train, fig in figs[pert_condition].items():
        add_context_annotation(
            ...
            perturbations={
                f"{pert_condition} impulse": (impulse_amplitude, impulse_start_step, impulse_end_step)
            },
            i_trial=i_trial,
...
figs = {
    ...
    for imp_condn, vels in all_vels_imp.items()
}
```
