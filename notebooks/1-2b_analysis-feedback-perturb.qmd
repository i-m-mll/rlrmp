...
```{python}
steady_pos = task_base.validation_trials.inits["mechanics.effector"].pos
...
```{python}   
all_measure_values = compute_all_measures(all_measures, all_responses_tuples)
...
figs = {
    ...
    for key, measure_by_pert_amp in all_measure_values.items()
}
```
...
figs = {
    ...
    for key, measure_by_pert_amp in all_measure_values_lohi.items()
}
```
...
```{python}
# TODO: Generalize with `plot.get_violins`. 
# Note that 
# 1) there is an additional level here, ImpulseAmpTuple, which is on the outside 
# but actually should be on the inside (since it's on the x axis); 
# 2) `TrainStdDict` is on the inside, but should probably be on the outside if we want 
# `get_violins` to accept an outer `Sequence` level, and plot it as side-by-side/split violins; 
# 3) `pert_var` is passed velow, but this isn't really necessary; we could pass `measure_data[pert_var]`
# just as well; so this isn't something that `get_violins` needs to handle
...
def get_violins_comparing_amplitudes(measure_data, measure_name, pert_var, annotation_kws=None, layout_kws=None):
    ...
    plot_disturbance_train_stds = disturbance_train_stds
...
figs = {
    ...
    for key, measure in all_measure_values.items()
}
```
