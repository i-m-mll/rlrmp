...
```{python}
def get_all_measures(measures: PyTree[Callable], all_responses: PyTree[Responses]):
    return jt.map(
        lambda func: jt.map(
...
all_measure_values = get_all_measures(all_measures, all_responses_tuples)

all_measure_values_lohi = {
    key: jt.map(
