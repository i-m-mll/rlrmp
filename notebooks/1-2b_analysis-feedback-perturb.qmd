...
controls_y_axes_labels = [fr'$F_{sub}$' for sub in components_labels[components_plot]]
...
figs = {
    imp_condn: {
        label: fbp.profiles(
            ...
            mode='std', 
            varname=fr"${controls_y_axes_labels[i]}$",
...
for path, fig in jax.tree_util.tree_flatten_with_path(figs, is_leaf=is_type(go.Figure))[0]:
    ...
    savefig(
        ...
        subdir=f"force_response/pert-{pert_condition}",
    )
...
```{python}
n = np.prod(jt.leaves(all_controls_plot_lohi)[0].shape[:-2])
...
vel_y_axes_labels = [fr'$v_{sub}$' for sub in components_labels[components_plot]]
...
figs = {
    imp_condn: {
        label: fbp.profiles(
            ...
            mode='std', 
            colors=disturbance_stds_train_colors_dark,
            varname=fr"${vel_y_axes_labels[i]}$",
            layout_kws=dict(
                legend=dict(
                    title="Train disturbance std.",
                ),
...
for path, fig in jax.tree_util.tree_flatten_with_path(figs, is_leaf=is_type(go.Figure))[0]:
    ...
    savefig(
        ...
        subdir=f"vel_response/pert-{pert_condition}",
    )
