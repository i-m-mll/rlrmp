...
impulse_xy_condition_keys = tree_key_tuples(
    impulse_conditions, keys_to_strs=True, is_leaf=lambda x: x is None,
)
...
disturbance_stds_train_colors = fbp.sample_colorscale_unique(disturbance_stds_train_colorscale, len(disturbance_stds_train))
disturbance_stds_train_colors_lohi = [disturbance_stds_train_colors[0], disturbance_stds_train_colors[-1]]
disturbance_stds_train_colors_dark = fbp.adjust_color_brightness(disturbance_stds_train_colors, darken_factor)
disturbance_stds_train_colors_dark_lohi = [disturbance_stds_train_colors_dark[0], disturbance_stds_train_colors_dark[-1]]
```
...
# therefore, we don't need to sample it 
pert_vars_colorscale = plc.qualitative.D3 
# pert_vars_colors = fbp.sample_colorscale_unique(pert_vars_colorscale, n_pert_vars)
pert_vars_colors = plc.convert_colors_to_same_type(pert_vars_colorscale, colortype='rgb')[0]
pert_vars_colors_lohi = [pert_vars_colors[0], pert_vars_colors[1]]
pert_vars_colors_dark = fbp.adjust_color_brightness(pert_vars_colors, darken_factor)
pert_vars_colors_dark_lohi = [pert_vars_colors_dark[0], pert_vars_colors_dark[1]]
```
...
#### Generate figures for each perturbation variables, comparing all training conditions

```{python}
all_controls_plot = impulse_responses[components_plot]['force']

controls_axes_labels = [fr'$F_{sub}$' for sub in components_labels[components_plot]]
...
figs = {
    imp_condn: {
        ...
        for i, label in enumerate(controls_axes_labels)
    }
...
for pert_condition in figs:
    print(pert_condition)
    for label, fig in figs[pert_condition].items():
        add_context_annotation(
            fig,
            perturbations={
                f"{pert_condition} ({analysis_variant} direction) impulse": (
                    impulse_amplitude[pert_condition], impulse_start_step, impulse_end_step
                )
            },
...
```{python}
all_controls_plot_lohi = jt.map(
    lambda controls_dict: subdict(controls_dict, disturbance_stds_train_lohi),
    all_controls_plot,
...
figs = {
    label: {
        imp_condn: fbp.profiles(
            tree_take_multi(vels, [i], [-1]),
            timesteps=jnp.arange(-n_preceding_steps, n_steps),
            mode='std', 
            colors=disturbance_stds_train_colors_dark_lohi,
            varname=fr"${label}$",
            layout_kws=dict(
                legend=dict(
                    title="Train disturbance std.",
                ),
            ),
            scatter_kws=dict(legend="legend"),
        )
        for imp_condn, vels in all_controls_plot_lohi.items()
    }
    for i, label in enumerate(controls_axes_labels)
}
...
def merge_profile_figs(figs):
    ...
    fig.add_traces(figs['pos'].data)
    toggle_bounds_visibility(fig)
    return fig
...
for label, fig in figs.items():
    ...
    add_context_annotation(
        ...
        perturbations={
            ...
            for pert_condition in impulse_amplitude
        },
        train_condition_strs=[f"{disturbance_type_train} force fields"],
        n=n,
    )
...
```{python}
all_vels_plot_lohi = jt.map(
    lambda vels_dict: subdict(vels_dict, disturbance_stds_train_lohi),
    all_vels_plot,
    is_leaf=is_type(OrderedDict),
)
...
figs = {
    label: {
        ...
        for imp_condn, vels in all_vels_plot_lohi.items()
    }
    for i, label in enumerate(axes_labels)
}
