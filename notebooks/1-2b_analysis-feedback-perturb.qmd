...
Our PyTrees will contain levels corresponding to training conditions (standard deviation of disturbance amplitude during training) and evaluation conditions (disturbance amplitudes during analysis).

Here, define some trivial subclasses of `dict` and `tuple` that can be identified by name, when manipulating these levels in the trees.
...
trained_models: dict[float, eqx.Module] = TrainStdDict(load(
...
if exclude_underperformers:
    trained_models = jt.map(
        lambda models, included: tree_take(models, jnp.where(included)[0], 0),
...
trained_models = TrainStdDict({key: trained_models[key] for key in include_disturbance_train_stds})
```
