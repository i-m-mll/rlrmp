...
if not load_from_parameters:
    fc = display_model_filechooser(MODELS_DIR, filter_pattern=f'{TRAIN_NB_ID}_*trained_models*')
```
...
```{python}
hyperparameters_filepath = models_filepath.replace(f'{BEST_MODEL_FILE_LABEL}.eqx', f'{HYPERPARAMS_FILE_LABEL}.json')
...
```{python}
fig_subdir = "aligned_to_reach_condition/compare_context_inputs"
```
...
```{python}
amplitude = disturbance_amplitude[disturbance_type]
...
for path, fig in tqdm(figs_flatten_with_paths(figs)):
    ...
    add_context_annotation(
        fig,
        perturbations={
            f"{disturbance_type}": (amplitude, None, None)
        },
        train_condition_strs=[
            f"{disturbance_type_train} fields with std {disturbance_train_std}",
            "the {training_method.upper()} context condition",
        ],        
    ...
    savefig(
        ...
        join([
            f"{training_method}",
            f"{disturbance_type}-field-{amplitude}",
        ]),
        subdir=fig_subdir,
    )  
```