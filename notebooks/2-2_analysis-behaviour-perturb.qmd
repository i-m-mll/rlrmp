...
```{python}
included_replicates = {
    variant_label: TrainStdDict({
        float(k): jnp.array(v) for k,v in d.items()
    })
    for variant_label, d in extras['included_replicates'].items()
}
...
best_replicate = {
    ...
    for variant_label, d in extras['best_replicate_by_loss'].items()
}
