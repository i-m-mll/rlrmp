...
from rnns_learn_robust_motor_policies.measures import output_corr
...
model_info = get_model_record(
    ...
    intervention_scaleup_batches=[1000,3000],
    # disturbance_stds=[0, 0.4, 0.8, 1.6, 2.0],
    state_reset_iterations=[500],
...
```{python}
def get_max_lateral_distance(aligned_vars, reach_length=None):
    distances = jnp.max(aligned_vars.pos[..., 1], axis=-1)
    if reach_length is not None:
        distances = 100 * distances / reach_length
    return distances
...
def end_position_error(aligned_vars, reach_length=1, last_n_steps=1):
...
all_measure_funcs, measure_handles = tree_unzip(OrderedDict({
    ...
    "Max lateral distance<br>(% reach length)": (
        partial(
            get_max_lateral_distance, 
            reach_length=example_task['full'].eval_reach_length,
        ), 
        "dist-lateral-max",
    ),
...
all_measures = get_all_measures(all_measure_funcs, aligned_vars['full'])
...
```{python}
n_dist = int(np.prod(jt.leaves(all_measures)[0].shape)) 
...
```{python}
n_dist = int(np.prod(jt.leaves(all_measures_subset_disturbance_train_stds)[0].shape))
...
```{python}
fig = get_violins_across_train_conditions(all_output_corrs['full'], "Output correlation", colors=disturbance_amplitudes_colors_dark)
```
