...
from rnns_learn_robust_motor_policies.state_utils import get_forward_lateral_vels, get_lateral_distances
...
```{python}
curl_amplitudes_test = [0.0, 0.25, 0.5, 1.0, 2.0]
n_steps = 100
workspace = ((-1., -1.),
             (1., 1.))
...
```{python}
forward_lateral_vels = jt.map(
    lambda state: get_forward_lateral_vels(state.mechanics.effector.vel, pos_endpoints),
    all_states,
    is_leaf=is_module,
)
```
...
for eval_curl_amplitude in curl_amplitudes_test:
    for i, label in enumerate(("Forward", "Lateral")):
        fig = fbp.profiles(
            tree_take(forward_lateral_vels[eval_curl_amplitude], i, -1), 
        ...
        filename = f"eval-curl-{eval_curl_amplitude}_{label.lower()}-vels"
        savefig(fig, filename, subdir=fig_subdir)
...
```{python}
max_forward_vels = jt.map(
    lambda vels: jnp.max(vels[..., 0], axis=-1),
    forward_lateral_vels,
)
```

```{python}
max_lateral_vels_l, max_lateral_vels_r = tree_unzip(jt.map(
    lambda vels: jt.map(
        lambda arr: jnp.max(arr, axis=-1),
        (vels[..., 1], -vels[..., 1]),
    ),
    forward_lateral_vels,
))
...
```{python}
lateral_distances = jt.map(
    lambda states: get_lateral_distances(states.mechanics.effector.pos, pos_endpoints),
    all_states,
    is_leaf=is_module,
)
```
...
```{python}
max_lateral_distances = jt.map(
    lambda distances: jnp.max(distances, axis=-1),
    lateral_distances,
)

sum_lateral_distances = jt.map(
    lambda distances: jnp.sum(distances, axis=-1),
    lateral_distances,
)

max_lateral_distances, sum_lateral_distances = jt.map(
    lambda arr: arr / eval_reach_length,
    (max_lateral_distances, sum_lateral_distances),
)
```
...
def end_position_error(states, trial_specs, last_n_steps=1):
...
def end_velocity_error(states, trial_specs, last_n_steps=1):
    final_vel = states.mechanics.effector.vel[..., -last_n_steps:, :]
    goal_vel = 0
    return jnp.mean(jnp.linalg.norm(final_vel - goal_vel, axis=-1), axis=-1)
    
all_end_pos_errors = jt.map(
    lambda states: end_position_error(states, trial_specs, last_n_steps),
    all_states,
    is_leaf=is_module,
)

all_end_vel_errors = jt.map(
    lambda states: end_velocity_error(states, trial_specs, last_n_steps),
    all_states,
    is_leaf=is_module,
)
```
...
```{python}
all_measures, measure_handles = tree_unzip({
    "Max. speed": (all_max_speeds, "speed-max"),
    "Max. forward velocity": (max_forward_vels, "vel-forward-max"),
    "Max. lateral velocity (left)": (max_lateral_vels_l, "vel-lateral-left-max"),
    "Max. lateral velocity (right)": (max_lateral_vels_r, "vel-lateral-right-max"),
    "Max. lateral distance": (max_lateral_distances, "dist-lateral-max"),
    "Sum lateral distance": (sum_lateral_distances, "dist-lateral-sum"),
    "End position error": (all_end_pos_errors, "error-end-pos"),
    "End velocity error": (all_end_vel_errors, "error-end-vel"),
})
```
...
```{python}
def get_violins_across_train_conditions(data, measure_name, eval_curl_amplitude):
    return go.Figure(
        data=[
            go.Violin(
        ...
        layout=dict(
            title=f"Response to magnitude {eval_curl_amplitude} curl field",
...
def get_one_measure_plot_per_eval_condition(plot_func, all_measures):
    return {
        measure_name: {
            eval_curl_amplitude: plot_func(
                measure[eval_curl_amplitude], measure_name, eval_curl_amplitude
            )
            for eval_curl_amplitude in measure
        }
        for measure_name, measure in all_measures.items()
    }

figs = get_one_measure_plot_per_eval_condition(get_violins_across_train_conditions, all_measures)
```
...
for measure_name, measure_handle in measure_handles.items():
    for eval_curl_amplitude, fig in figs[measure_name].items():
        savefig(
            fig, 
            '_'.join([
                f"{measure_handle}",
...
```{python}
labels = ["No curl during training", "Largest training curl std."]
colors = ['blue', 'orange']
train_curl_stds_to_compare = [0, 0.5]

def get_measure_replicate_comparisons(data, measure_name, eval_curl_amplitude):
