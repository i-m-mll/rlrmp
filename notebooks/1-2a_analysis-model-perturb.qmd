...
```{python}
disturbance_train_stds = disturbance_train_stds_load[:-1]  # Change nothing.
...
```{python}
plot_states = tree_take(all_states_small, i_condition, 2)
```
...
for disturbance_amplitude in tqdm(figs):
    for disturbance_train_std, fig in figs[disturbance_amplitude].items():
        ...
        add_context_annotation(
            ...
            n=n_replicates_included[disturbance_train_std],  # color/legend is by trial, so this is N per trial
...
for disturbance_train_std, fig in tqdm(figs.items()):
    n_trials = n_evals_small * n_replicates_included[disturbance_train_std]
