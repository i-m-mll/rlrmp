...
for eval_curl_amplitude in curl_amplitudes_test:
    for i, label in enumerate(("Forward", "Lateral")):
        fig = fbp.profiles(
            tree_take(forward_lateral_vel[eval_curl_amplitude], i, -1), 
...
def get_max_forward_vel(states):
    ...
    xx = get_forward_lateral_vel(pos, pos_endpoints)[..., 0]
    return jnp.max(xx, axis=-1)
```

```{python}
def get_max_lateral_vel(states, mul=1.):
    pos = states.mechanics.effector.pos
    return jnp.max(mul * get_forward_lateral_vel(pos, pos_endpoints)[..., 1], axis=-1)
```
...
def get_argmax_forward_vel(states):
    pos = states.mechanics.effector.pos
    return jnp.argmax(get_forward_lateral_vels(pos, pos_endpoints)[..., 0], axis=-1)
```
...
```{python}
all_measure_funcs, measure_handles = tree_unzip({
...
def get_violins_across_train_conditions(data, measure_name, eval_curl_amplitude):
    ...
    return go.Figure(
        ...
        layout=dict(
            title=(f"Response to magnitude {eval_curl_amplitude} curl field <br>N = {n_dist}"),
...
for measure_name, measure_handle in measure_handles.items():
    for eval_curl_amplitude, fig in figs0[measure_name].items():
        savefig(
            fig, 
            '_'.join([
                f"{measure_handle}",
...
labels = [
    ...
    f"Training curl magnitude ~ \U0001d4dd(0,{upper_train_curl_std})",
]
colors = ['blue', 'orange']
train_curl_stds_to_compare = [0, upper_train_curl_std]
n_dist = n_trials * task.n_validation_trials
