...
from feedbax.misc import git_commit_id, attr_str_tree_to_where_func, lohi
...
trained_models = dict(zip(disturbance_train_stds, trained_models))
```
...
```{python}
def load_model_parameter_histories(models_filepath):
    # Wrap in a function to avoid polluting the global namespace
    model_parameter_histories_load, train_hyperparameters = load_with_hyperparameters(
    ...
    model_parameter_histories = dict(zip(disturbance_train_stds, model_parameter_histories_load))
    return model_parameter_histories, train_hyperparameters
    
model_parameter_histories, train_hyperparameters = load_model_parameter_histories(
    models_filepath,
)
...
# for disturbance_train_std, i in load_spec.items():
#     trained_models[disturbance_train_std] = swap_model_trainables(
#         trained_models[disturbance_train_std], 
#         tree_take(model_parameter_histories[disturbance_train_std], i), 
#         where_train,
#     )
```
...
We will generally evaluate on a 2*2 grid of center-out reach sets (i.e. 4 sets total), with 24 reach directions per set. 
...
if disturbance_type == 'curl':
    ...
    def disturbance(field_std):
        # Could do random direction; see 1-1.             
        return FixedField.with_params(
            scale=field_std,
            field=jnp.array([0.0, 1.0]),  # to the right
        ) 
...
In particular, for each trained ensemble of models (i.e. each training condition), evaluate each model in the ensemble on `n_evals` repetitions of each of the reach conditions. 
...
@eqx.filter_jit
def vmap_eval_ensemble(models, task, n_evals):
    # TODO: might save compulation if we modify the task here...
    return eqx.filter_vmap(get_eval_ensemble(models, task))(
        jr.split(key_eval, n_evals)
    )
```
...
2. Vmap over `keys_eval`, to obtain `n_evals` different evaluations of the center-out set. 
3. Use `jt.map` to repeat for each training condition (entry in `models`).

Now, evaluate the trained models on each of the task variants. First, define the full evaluation as a function so that we can estimate the memory needed for the result. 
...
print(f"Estimate {sum(all_states_bytes) / 1e9:.2f} GB of memory needed to evaluate all states.")
```
...
The objects `all_states` and `all_states_small` are dicts whose keys are the `disturbance_amplitudes`, and whose values are dicts whose keys are the `disturbance_train_stds` used in training, and whose values are the PyTrees of state arrays.
...
all_measures_subset_disturbance_train_stds = {
    k1: {k2: subdict(v, lohi(disturbance_train_stds)) for k2, v in all_measures[k1].items()}
    for k1 in all_measures
}

box_measures_subset_disturbance_train_stds = {
    k1: {k2: subdict(v, lohi(disturbance_train_stds)) for k2, v in box_measures[k1].items()}
...
### Plot comparison of distributions for zero vs. high train disturbance std, for different replicates
