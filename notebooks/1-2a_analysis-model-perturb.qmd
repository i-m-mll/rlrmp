...
```{python}
def get_violins_across_train_conditions(data, measure_name, curl_amplitude, annotation_kws=None):
    ...
    add_context_annotation(fig, curl_amplitude=curl_amplitude, n=n_dist, **annotation_kws)
    ...
    return {
        measure_name: {
            curl_amplitude: plot_func(
                measure[curl_amplitude], measure_name, curl_amplitude, **kwargs
            )
...
def get_measure_replicate_comparisons(data, measure_name, curl_amplitude, annotation_kws=None):
    n_dist = np.prod(jt.leaves(data)[0].shape)
...
```{python}
all_measures_lohi = {
    measure_name: subdict(measure, curl_amplitudes_lohi)
    ...
    fig = go.Figure(
        data=jt.leaves([
            [
                ...
                for j, (train_curl_std, data) in enumerate(measure_data[curl_amplitude].items())
            ]
            for i, curl_amplitude in enumerate(measure_data)
        ]),
        layout=dict(
            width=500,
            height=400,
