...
trials_colors, trials_colors_dark = get_colors_dicts(
    range(n_trials), COLORSCALES['trials'],
)
...
@eqx.filter_jit
def vmap_eval_ensemble(models, task, n_trials):
...
all_states = jt.map(
    lambda task: jt.map(
        lambda models: vmap_eval_ensemble(models, task, n_trials),
        trained_models,
        is_leaf=is_module,
    ),
    tasks,
...
all_states_small = jt.map(
    lambda task: jt.map(
        lambda models: vmap_eval_ensemble(models, task, n_trials_small),
        trained_models,
        is_leaf=is_module,
    ),
    tasks_small,
