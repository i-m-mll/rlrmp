...
We may want to adjust the noise level of a model trained at a different level. 

Set these to `None` to keep the value used during training.
...
train_disturbance_type = 'curl'
```

```{python}
#| tags: [parameters]
train_fb_noise_std = 0.2
train_motor_noise_std = 0.2
train_fb_delay_steps = 0  # TODO: it doesn't make sense to change the delay between train and test! get rid of this
```

```{python}
train_fb_noise_std = float(train_fb_noise_std)
train_motor_noise_std = float(train_motor_noise_std)
```

```{python}
train_suffix = '_'.join([
    f"{train_disturbance_type}",
    f"noise-{train_fb_noise_std}-{train_motor_noise_std}",
    f"delay-{train_fb_delay_steps}",
])

# raise RuntimeError(type(train_fb_noise_std))

if load_from_parameters and 'None' in train_suffix:
...
if not load_from_parameters:
    ...
    models_filepath = str(find_unique_filepath(MODELS_DIR, f"1-1__{train_suffix}__trained_models"))
    if models_filepath is None:
        raise FileNotFoundError(f"No models found with suffix: {train_suffix}")
...
### Modify the system noise and delay if needed
...
train_curl_stds = hyperparameters['disturbance_stds']
...
### Assign some variables to indicate which hyperparameters we ended up using

When `load_from_parameters == True` then some of the following is redundant because we'll have specified it already. However we still need to do it in the other case. 

```{python}
train_suffix = models_filepath.split('__')[1]
train_disturbance_type = hyperparameters['disturbance_type']
train_noise_stds = dict(
...
If we didn't load from parameters, then can infer that the evaluation conditions will be the same as the training conditions.

```{python}
if disturbance_type is None:
    disturbance_type = train_disturbance_type

noise_stds = {
    k: v if v is not None else train_noise_stds[k]
    for k, v in noise_stds.items()
}

if feedback_delay_steps is None:
    feedback_delay_steps = train_fb_delay_steps
```
...
```{python}
include_curl_stds = train_curl_stds
...
## Additional setup that depends on loaded models

The figure hierarchy will depend on the hyperparameters of the trained model, so we needed to load the trained model before setting up the figure subdirectory.

```{python}
suffix = f"{disturbance_type}_noise-{noise_stds['feedback']}-{noise_stds['motor']}_delay-{feedback_delay_steps}"

FIGS_DIR = Path(f'../figures/{NB_ID}/train__{train_suffix}/{suffix}')
...
plot_states_stacked = {
    disturbance_amplitude: tree_stack(subdict(states, train_curl_stds_plot).values())
...
def get_violins_across_train_conditions(data, measure_name, disturbance_amplitude, annotation_kws=None):
    ...
    add_context_annotation(fig, disturbance_amplitude=disturbance_amplitude, n=n_dist, **annotation_kws)
...
all_measures_lohi = {
    ...
    for measure_name, measure in all_measures_subset_train_curl_stds.items()
}
```
...
```{python}
del all_states, all_states_small, all_measures, figs
```