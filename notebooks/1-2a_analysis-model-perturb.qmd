...
trials_colorscale = 'Blugrn'  # for trials
...
curl_stds_train_colorscale = 'plotly3'
curl_stds_train_colors = plc.sample_colorscale(curl_stds_train_colorscale, len(curl_stds_train) + 1)[:-1]
curl_stds_train_colors_lohi = [curl_stds_train_colors[0], curl_stds_train_colors[-1]]

# when coloring by evaluation condition
disturbance_amplitudes_colorscale = 'viridis'
disturbance_amplitudes_colors = plc.sample_colorscale(disturbance_amplitudes_colorscale, len(disturbance_amplitudes) + 1)[:-1]
...
curl_stds_train_plot = curl_stds_train_lohi
...
figs = jt.map(
    lambda states: fbp.trajectories_2D(
        ...
        legend_labels=curl_stds_train_plot,
    ...
    fig = go.Figure(
        data=[
            go.Violin(
                ...
                line_color=curl_stds_train_colors[i],
                showlegend=False,
                spanmode='hard',
...
colors = curl_stds_train_colors_lohi
        ...
        data=jt.leaves([
            [
                go.Violin(
                    ...
                    box_visible=False,
                    meanline_visible=True,
                    line_color=disturbance_amplitudes_colors_lohi[i],
                    showlegend=(j == 0),
                    spanmode='hard',
                )
