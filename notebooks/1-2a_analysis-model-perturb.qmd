...
Here, define some trivial subclasses of `dict` that can be identified by name, when manipulating these levels in the trees.
...
for disturbance_amplitude in tqdm(figs):
    for disturbance_train_std, fig in figs[disturbance_amplitude].items():
        ...
        add_context_annotation(
            ...
            n=n_replicates_included[disturbance_train_std] * n_conditions_small,  # color/legend is by trial, so this is N per trial
...
for disturbance_train_std, fig in tqdm(figs.items()):
    ...
    add_context_annotation(
        fig,
        train_condition_strs=[
            f"{disturbance_type_train} fields with amplitude ~ \U0001d4dd(0,{disturbance_train_std})"
        ],
        n=n_trials,
    )
...
def get_violins_across_train_conditions(measure_data, measure_name, annotation_kws=None):
    ...
    fig = go.Figure(
        layout=dict(
            ...
            xaxis_title="Train field std.",
            xaxis_type='category',
            # xaxis_range=[-0.5, n_train_std - 0.5],
