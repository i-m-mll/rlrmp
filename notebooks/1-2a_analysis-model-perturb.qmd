...
fc.default_filename = get_latest_matching_file(MODELS_DIR, fc.filter_pattern)
params_widget = HTML("")
...
Make some subsets of training conditions for reduced plots:

```{python}
train_curl_stds_lohi = [0, train_curl_stds[-1]]
```
...
```{python}
curl_amplitudes = [0.0, 0.5, 1.0, 2.0, 4.0]
curl_amplitudes_lohi = [0.0, curl_amplitudes[-1]]
```
...
tasks, _ = tree_unzip(jt.map(
    ...
    dict(zip(curl_amplitudes, curl_amplitudes)),
))
...
The objects `all_states` and `all_states_small` are dicts whose keys are the `curl_amplitudes`, and whose values are dicts whose keys are the `curl_stds` used in training, and whose values are the PyTrees of state arrays.
...
**TODO: Plot inits/goals and straight guides -- generalize the code used a bit later in this nb to generate traces**
...
Add an annotation to provide context, and save figures.

```{python}
for curl_amplitude in figs:
    for train_curl_std, fig in figs[curl_amplitude].items():
        add_context_annotation(
            fig,
...
Display just the (2x2) zero versus high curl, training versus evaluation conditions.
...
for train_curl_std in figs:
    fig = figs[train_curl_std]
...
train_curl_stds_plot = train_curl_stds_lohi
...
plot_states_stacked = {
    curl_amplitude: tree_stack(subdict(states, train_curl_stds_plot).values())
    for curl_amplitude, states in plot_states.items()
}
...
for curl_amplitude, fig in figs.items():
    add_context_annotation(
        fig,
        curl_amplitude=curl_amplitude,
        n=n_trials_plot,
    ...
    savefig(
        ...
        '_'.join([
            f"single-condition-{i_condition}",
            f"curl_amplitude-{curl_amplitude}",
        ]),
        subdir=fig_subdir,
    )   
```
...
figs = {
    curl_amplitude: fbp.profiles(
        speeds,
        mode='std', 
        varname="Speed",
    )
    for curl_amplitude, speeds in all_speeds.items()
}
...
figs = {
    curl_amplitude: {
        label: fbp.profiles(
            tree_take(forward_lateral_vel, i, -1)[curl_amplitude],
    ...
    for curl_amplitude in curl_amplitudes
}

for curl_amplitude in curl_amplitudes:
    for i, label in enumerate(("Forward", "Lateral")):
        fig = figs[curl_amplitude][label]
        add_context_annotation(
            fig,
            curl_amplitude=curl_amplitude,
            n=n_dist,
        )
        filename = f"eval-curl-{curl_amplitude}_{label.lower()}-vels"
        savefig(fig, filename, subdir=fig_subdir)
        
for curl_amplitude in curl_amplitudes_lohi:
    figs[curl_amplitude]["Forward"].show()
    figs[curl_amplitude]["Lateral"].show()
...
def end_position_error(states, last_n_steps=1):
    ...
    return jnp.mean(jnp.linalg.norm(final_pos - goal_pos, axis=-1), axis=-1)
...
all_measure_funcs, measure_handles = tree_unzip(OrderedDict({
    ...
    "End position error": (
        partial(end_position_error, last_n_steps=last_n_steps), 
        "error-end-pos",
    ),
    "End velocity error": (
...
```{python}
def get_violins_across_train_conditions(data, measure_name, curl_amplitude):
    ...
    return go.Figure(
        data=[
            ...
            for train_curl_std in data
        ],
        layout=dict(
            title=(f"Response to amplitude {curl_amplitude} curl field <br>N = {n_dist}"),
...
def get_one_measure_plot_per_eval_condition(plot_func, all_measures):
    return {
        measure_name: {
            curl_amplitude: plot_func(
                measure[curl_amplitude], measure_name, curl_amplitude
            )
            for curl_amplitude in measure
        }
...
```{python}
fig_subdir = "measure_distributions/compare_train_condns"

for measure_name, measure_handle in measure_handles.items():
    for curl_amplitude, fig in figs[measure_name].items():
        savefig(
            fig, 
            '_'.join([
                f"{measure_handle}",
...
figs0 = get_one_measure_plot_per_eval_condition(get_violins_across_train_conditions, all_measures_one_dirxn)
```

```{python}
fig_subdir = "measure_distributions/compare_train_condns_onedirxn"

for measure_name, measure_handle in measure_handles.items():
    for curl_amplitude, fig in figs0[measure_name].items():
        savefig(
            fig, 
            '_'.join([
                f"{measure_handle}",
...
```{python}
for measure_name in all_measures:
    figs0[measure_name][2].show()
```

### Repeat for just the zero vs. highest-std training condition

```{python}
upper_train_curl_std = train_curl_stds[-1]
train_curl_stds_to_compare = [0, upper_train_curl_std]

all_measures_subset_train_curl_stds = {
    k1: {k2: subdict(v, train_curl_stds_to_compare) for k2, v in all_measures[k1].items()}
...
```{python}
labels = [
    "No curl during training", 
    f"Training curl amplitude ~ \U0001d4dd(0,{upper_train_curl_std})",
]
colors = ['blue', 'orange']


def get_measure_replicate_comparisons(data, measure_name, curl_amplitude):
    n_dist = np.prod(jt.leaves(data)[0].shape)
...
```{python}
fig_subdir = "measure_distributions/compare_train_condn_extremes_by_replicate"
...
### Comparison of no-curl vs. high-curl evaluation, for no-curl vs. high-curl training
