...
def get_ss_rnn_func_at_context(pos, context, rnn):
...
def get_ss_rnn_fps(pos, rnn, candidate_states, context):
    fps = fpf_func(
        get_ss_rnn_func_at_context(pos, context, rnn), 
        candidate_states, 
        fp_tol,
    )
    return fps
...
Get the top 5 points for each replicate and context input.
...
:::{note}
More interestingly, sometimes more than one "fixed point" meets all criteria and is included in `all_top_fps`. But typically these are actually the same fixed point, except that due to differences in optimization, some context inputs converged better than others. 
...
### Find fixed points at a grid of steady-state positions
...
### Linearization at a set of grid points

**TODO**: Repeat the last section for the grid steady-state FPs.
