...
# def multi_vmap(func, in_axes: Sequence[PyTree[int | Callable[[Any], int] | None]]):
#     """Given a sequence of `in_axes`, construct a nested vmap of `func`."""
#     result = func
#     for ax in in_axes:
#         result = eqx.filter_vmap(result, in_axes=ax)
#     return result
    
def multi_vmap(func, in_axes_sequence):
    """Given a sequence of in_axes, construct nested vmaps."""
    if not in_axes_sequence:
        return func
    return eqx.filter_vmap(
        multi_vmap(func, in_axes_sequence[1:]),
        in_axes=in_axes_sequence[0]
    )
...
def get_ss_fps_at_positions(
    ...
    candidates = jt.map(
        lambda states: jnp.reshape(
            states.net.hidden, 
            (model_info.n_replicates, -1, model_info.hidden_size),
        )[:, ::stride_candidates],
        all_states,
        is_leaf=is_module,
    )
...
all_states_hidden_flat = jnp.concatenate(jt.leaves(
    jt.map(lambda states: jnp.reshape(states, (-1, model_info.hidden_size)), all_states_hidden)  
))
```
...
for path, fig in tqdm(figs_flatten_with_paths(eigval_figs)):
    fig_params = dict(
        ...
        n=model_info.hidden_size * model_info.n_replicates * eval_grid_n ** 2,
    )
...
for path, fig in tqdm(figs_flatten_with_paths(eigval_figs)):
    fig_params = dict(
        ...
        disturbance_type_train=model_info.disturbance_type,
        disturbance_train_std=path[1].key,
        n=model_info.hidden_size * len(context_inputs) * eval_grid_n ** 2,
    )
...
for path, fig in tqdm(figs_flatten_with_paths(eigval_figs)):
    fig_params = dict(
        ...
        disturbance_type_train=model_info.disturbance_type,
        disturbance_train_std=path[1].key,
        n=model_info.hidden_size * len(context_inputs) * model_info.n_replicates,
    )
...
```{python}
perturbation_amplitudes = jnp.array([0.01, 0.1, 1])

n_perturbation_directions = 8
```
...
@partial(eqx.filter_vmap, in_axes=(None, None, 0, 0, None)) # Over replicates
@partial(eqx.filter_vmap, in_axes=(0, None, None, 0, None)) # Over grid positions
@partial(eqx.filter_vmap, in_axes=(None, 0, None, None, None)) # Over perturbation directions
@partial(eqx.filter_vmap, in_axes=(None, 0, None, None, None)) # Over perturbation amplitudes
def get_rnn_fp_with_fb_pert(pos, fb_pert, rnn_cell, candidate_states, context):
    """Find the fixed point """       
    fps = fpf_func(
        get_rnn_func(pos, context, fb_pert, rnn_cell), 
        candidate_states[None, :], # Since we are vmapping over grid positions, there's only a single candidate 
        fp_tol,
    )
    return fps
...
```{python}
fps_pert_shape = eqx.filter_eval_shape(
    get_fps_at_positions_with_fb_perts,
    *args,
)

print(f"Estimate {tree_struct_bytes(fps_pert_shape) / 1E6} MB of memory needed for all FPs.")
```

```{python}
# leaf shape: (replicates, grid points, perturbation amplitudes, perturbation directions, fp candidates (=1), hidden size)
fps_pert = get_fps_at_positions_with_fb_perts(*args)
```

As earlier, assume that if there are multiple FPs returned, they are actually the same FP due to slight variations in convergence. Average over them.
...
Supplementary: is it consistent across grid points? Across replicates?

### Analysis of eigenspectra of Jacobians
