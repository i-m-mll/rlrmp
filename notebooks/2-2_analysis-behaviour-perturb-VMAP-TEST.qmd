...
training_methods = ['amplitude', 'std']  # To analyze; suset of `training_methods_load`
```
...
```{python}
models_base, model_info, replicate_info, n_replicates_included = query_and_load_models(
    db_session,
    setup_task_model_pairs,
    params_query=dict(
        origin=TRAIN_NB_ID,
...
```{python}
from feedbax._tree import apply_to_filtered_leaves as atfl

tree_stack

all_models = jt.map(
    lambda d: tree_stack(list(d.values())), 
    models_base, 
...
```{python}
def test(disturbance_amplitude):
    task, model = schedule_intervenor(  # (implicitly) over train stds
...
```{python}
task_base, models = tree_unzip(jt.map( # over training methods
    lambda models_by_method: jt.map( # over disturbance amplitudes
