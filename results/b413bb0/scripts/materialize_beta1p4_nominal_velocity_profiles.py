#!/usr/bin/env python
"""Materialize beta 1.4 nominal velocity profiles for b413."""
...
ISSUE = "b413bb0"
TOPIC = "beta1p4_nominal_velocity_profiles"
...
RUN_ROWS = (
    {
        ...
        "legend": "Direct epsilon GRU",
        "color": "#2563eb",
        "adversary_mechanism": "direct_epsilon",
    },
    {
        "run_id": "linear_no_bias",
        "label": "Linear no-bias",
        "legend": "Linear no-bias GRU",
        "color": "#7c3aed",
...
def materialize_figure(*, run_profiles: Mapping[str, np.ndarray], extlqg_context: Mapping[str, Any], robust_context: Mapping[str, Any]) -> dict[str, Any]:
    """Build and save the beta 1.4 nominal velocity figure."""
    ...
    fig = build_nominal_profile_figure(rows=RUN_ROWS, add_run_profiles=add_runs, ext_profile=forward_velocity_profile(extlqg_context["base_evaluation"].velocity), robust_profile=forward_velocity_profile(robust_context["velocity"]), comparator_colors=COMPARATOR_COLORS, title="b413 beta 1.4 nominal forward velocity profiles", height=760)
    ...
    return {"status": "materialized", "topic": TOPIC, "save_result": json_safe(saved), "spec": repo_rel(SPEC_PATH), "html": repo_rel(HTML_PATH), "png": repo_rel(PNG_PATH) if png_export["status"] == "written" else None, "png_export": png_export, "rows": [{"run_id": row["run_id"], "label": row["label"], "adversary_mechanism": row["adversary_mechanism"]} for row in RUN_ROWS], "analytical_comparator_contract": spec["analytical_comparator_contract"]}
...
def build_spec(*, robust_context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_nominal_velocity_spec(
        schema_version="rlrmp.b413_beta1p4_nominal_velocity_profiles.v1", issue=ISSUE,
        figure_kind="beta1p4_nominal_forward_velocity_profiles", robust_contract=robust_context["contract"], inputs=inputs,
...
def write_note(output: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# Beta 1.4 Nominal Velocity Profiles",
        "",
