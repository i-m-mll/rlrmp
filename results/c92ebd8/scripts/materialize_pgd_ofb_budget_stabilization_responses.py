"""Materialize c92 output-feedback-budget PGD stabilization responses."""
...
ISSUE = "c92ebd8"
TOPIC = "pgd_ofb_budget_stabilization_perturbation_responses"
MARKER = TOPIC
SCHEMA_VERSION = "rlrmp.c92ebd8.pgd_ofb_budget_stabilization_responses.v1"
DETAIL_SCHEMA_VERSION = "rlrmp.c92ebd8.pgd_ofb_budget_stabilization_detail.v1"

BUDGET_COLUMNS: tuple[tuple[str, str], ...] = (
    ("ofb1p05", "moderate_pgd_ofb1p05"),
    ("ofb1p4", "moderate_pgd_ofb1p4"),
)
ROW_SPECS: tuple[base.RowSpec, ...] = (
    base.RowSpec("open_loop_moderate", "no_pgd_open_loop", "moderate"),
...
SOURCE_STYLES = {
    ...
    "pgd_ofb1p05_gru": {
        "label": "PGD OFB gamma 1.05 budget GRU",
        "color": "#7c3aed",
        "band": "rgba(124,58,237,0.10)",
    },
    "pgd_ofb1p4_gru": {
        "label": "PGD OFB gamma 1.4 budget GRU",
        "color": "#be185d",
        "band": "rgba(190,24,93,0.10)",
    },
    "extlqg6d": {
        "label": "6D extLQG",
        "color": "#c2410c",
...
def main() -> None:
    """Run local materialization and write the requested c92 outputs."""

    repo_root = Path(REPO_ROOT).resolve()
    summary = materialize_row_detail(repo_root=repo_root)
...
def materialize_row_detail(*, repo_root: Path) -> dict[str, Any]:
    """Evaluate the no-PGD baseline and two output-feedback-budget PGD rows."""

    rows = [base.evaluate_row(row_spec, repo_root=repo_root) for row_spec in ROW_SPECS]
    return {
        "schema_version": DETAIL_SCHEMA_VERSION,
        "issue": ISSUE,
        "source_experiment": ISSUE,
        "task_label": "stabilization task endpoint response",
        "rows_requested": [row.run_id for row in ROW_SPECS],
        ...
        figure_specs.append(
            {
                ...
                "figure_kind": "stabilization_task_pgd_ofb_budget_response_grid",
                "layout": {
                    "rows": 3,
                    "cols": 2,
...
def build_family_figure(
    *,
    ...
    """Build one response-state-by-budget grid."""

    def cell_context(response_variable: str, column: tuple[str, str]) -> dict[str, Any]:
        budget, pgd_run_id = column
        timing = summary_by_run["open_loop_moderate"]["timing"]
        dt = float(summary_by_run["open_loop_moderate"]["dt_s"])
        baseline_rows = family_rows(detail=detail, run_id="open_loop_moderate", family=family)
        learned = []
        for source, run_id in (
            ...
            learned.append({"source": source, "run_id": run_id, "profile": profile})
        return {
            "timing": timing, "dt": dt,
            "baseline_rows": baseline_rows, "learned": learned,
            "cache_prefix": (), "coverage_metadata": {"budget": budget},
            "identity_metadata": {"budget_column": budget},
        }
    ...
    return canonical_build_family_figure(
        ...
        analytical_profile=lambda **kwargs: analytical_profile(
            extlqg_context=extlqg_context, robust_context=robust_context, **kwargs
        ),
        ...
        title=f"c92 output-feedback-budget PGD stabilization task response: {base.FAMILY_LABELS[family]}",
        width=1180,
...
def build_spec(
    *,
    ...
    return {
        ...
        "bulk_detail": base.repo_relative(detail_path, repo_root),
        "bulk_figure_dir": base.repo_relative(figure_dir, repo_root),
        "plot_contract": {
            ...
            "grid": "three response-state rows by two PGD output-feedback-budget columns",
            "row_axis": "response_state",
            "row_order": list(base.RESPONSE_VARIABLE_ORDER),
            "col_axis": "pgd_output_feedback_budget",
            "col_order": [column for column, _run_id in BUDGET_COLUMNS],
...
def validate_spec(spec: Mapping[str, Any]) -> None:
    ...
    if int(spec["png_count"]) != len(base.PERTURBATION_FAMILY_ORDER):
        raise ValueError(f"expected 5 PNGs, got {spec['png_count']}")
    for figure in figures:
        layout = figure["layout"]
        if (layout["rows"], layout["cols"]) != (3, 2):
            raise ValueError(f"{figure['family']} should be 3x2, got {layout}")
        if list(layout["row_order"]) != list(base.RESPONSE_VARIABLE_ORDER):
            raise ValueError(f"{figure['family']} response row order mismatch")
        if list(layout["col_order"]) != [column for column, _run_id in BUDGET_COLUMNS]:
            raise ValueError(f"{figure['family']} budget column order mismatch")
        common_sources = {"no_pgd_gru", "extlqg6d"}
        ...
        for row in figure["coverage"]:
            seen[(row["response_variable"], row["budget_column"])].add(row["source"])
        ...
        for response in base.RESPONSE_VARIABLE_ORDER:
            for budget, _run_id in BUDGET_COLUMNS:
                ...
                expected_sources.add(f"pgd_{budget}_gru")
        ...
        if len(markers) != 6:
            raise ValueError(f"{figure['family']} should have 6 event markers, got {len(markers)}")
...
def render_note(spec: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "- Rows: shared baseline `open_loop_moderate`, PGD rows "
        "`moderate_pgd_ofb1p05` and `moderate_pgd_ofb1p4`.",
        "- Figure family: one figure per perturbation family "
        f"({', '.join(f'`{family}`' for family in base.PERTURBATION_FAMILY_ORDER)}).",
        "- Per-figure layout: response-state rows (`command`, `position`, `velocity`) "
        "by PGD-budget columns (`ofb1p05`, `ofb1p4`).",
