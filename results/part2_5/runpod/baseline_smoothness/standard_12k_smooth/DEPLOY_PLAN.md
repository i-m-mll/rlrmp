...
- Existing baseline `_artifacts/part2_5/runpod/baseline/standard_12k/` ran on this exact config (modulo the smoothness term) on a GPU pod and produced a 4.0 MiB model in ~93 min wall-clock — peak VRAM was well below 24 GB.
...
The `run.json` will land at `results/part2_5/runpod/baseline_smoothness/standard_12k_smooth/run.json` automatically via the script's mirror-invariant logic.
