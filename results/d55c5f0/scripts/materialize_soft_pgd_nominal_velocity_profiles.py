"""Materialize d55 soft-PGD nominal velocity profiles."""
...
TOPIC = "soft_pgd_nominal_velocity_profiles"
...
def main() -> None:
    ...
    assert_soft_inputs_ready()
    nominal_base = load_nominal_reference()
    extlqg_context = _build_extlqg_comparator_context(physical_dim=6)
    robust_context = nominal_base.build_robust_output_feedback_6d_context()
    run_profiles = evaluate_requested_run_profiles(nominal_base=nominal_base)
...
def load_nominal_reference() -> Any:
    """Load c92 nominal helper functions without inheriting c92 paths."""

    return load_c92_module(
        "c92_pgd_1p05_nominal_velocity_profiles",
        "materialize_pgd_1p05_nominal_velocity_profiles.py",
    )


def evaluate_requested_run_profiles(*, nominal_base: Any) -> dict[str, np.ndarray]:
    ...
    for run in runs:
        ...
        profiles[run.run_id] = nominal_base.forward_velocity_profile(evaluation.velocity)
    return profiles


def materialize_figure(*, run_profiles: Mapping[str, np.ndarray], extlqg_context: Mapping[str, Any], robust_context: Mapping[str, Any], nominal_base: Any) -> dict[str, Any]:
    """Build and save the one-panel d55 soft-PGD nominal velocity figure."""
    panel_rows = [{"label": "Soft-PGD first batch"}]
    def add_runs(fig: go.Figure, _row_spec: Mapping[str, Any], row_index: int) -> None:
        for row in SOFT_ROWS:
            add_mean_band(fig, run_profiles[row.run_id], row=row_index, col=1, name=row.legend, color=row.color, showlegend=True)
    fig = build_nominal_profile_figure(rows=panel_rows, add_run_profiles=add_runs, ext_profile=nominal_base.forward_velocity_profile(extlqg_context["base_evaluation"].velocity), robust_profile=nominal_base.forward_velocity_profile(robust_context["velocity"]), comparator_colors=COMPARATOR_COLORS, title="d55 soft-PGD nominal forward velocity profiles", height=560)
    ...
    return {"status": "materialized", "topic": TOPIC, "save_result": json_safe(saved), "spec": repo_rel(SPEC_PATH), "html": repo_rel(HTML_PATH), "png": repo_rel(PNG_PATH) if png_export["status"] == "written" else None, "png_export": png_export, "rows": [{"run_id": row.run_id, "label": row.label, "gamma_factor": row.gamma_factor} for row in SOFT_ROWS], "analytical_comparator_contract": spec["analytical_comparator_contract"]}


def build_spec(*, robust_context: Mapping[str, Any]) -> dict[str, Any]:
    """Return the tracked figure specification."""
    inputs = [{"role": "run_spec", "path": f"results/{ISSUE}/runs/{row.run_id}.json", "label": row.label, "gamma_factor": row.gamma_factor} for row in SOFT_ROWS]
    return build_nominal_velocity_spec(
        schema_version="rlrmp.d55_soft_pgd_nominal_velocity_profiles.v1", issue=ISSUE,
        figure_kind="soft_pgd_nominal_forward_velocity_profiles", robust_contract=robust_context["contract"], inputs=inputs,
        transform_name="final_checkpoint_forward_velocity_profile_mean_band",
        transform_kwargs={"row_labels": {row.run_id: row.label for row in SOFT_ROWS}, "n_rollout_trials": DEFAULT_N_ROLLOUT_TRIALS, "use_validation_selected_checkpoints": False, "analytical_comparators": ["6d_output_feedback_extlqg", "6d_output_feedback_hinf"]},
        rows=1, cols=1, outputs={"html": repo_rel(HTML_PATH), "png": repo_rel(PNG_PATH)},
    )
...
def write_note(output: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# Soft-PGD Nominal Velocity Profiles",
        "",
        "- Scope: one nominal velocity subplot containing all three d55 soft-PGD "
        "first-batch GRU rows.",
