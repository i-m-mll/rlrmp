...
def materialize_figure(
    *,
    ...
    fig = profile_comparison_grid(
        ...
        rows=1,
        cols=1,
        subplot_titles=["Soft-PGD first batch"],
        vertical_spacing=0.075,
    )
    ext_profile = nominal_base.forward_velocity_profile(extlqg_context["base_evaluation"].velocity)
    robust_profile = nominal_base.forward_velocity_profile(robust_context["velocity"])

    for row in SOFT_ROWS:
        add_mean_band(
            fig,
            run_profiles[row.run_id],
            row=1,
            col=1,
            name=row.legend,
            color=row.color,
    ...
    fig.update_xaxes(title_text="time from movement onset (s)", row=1, col=1)
    fig.update_layout(
        title="d55 soft-PGD nominal forward velocity profiles",
...
def build_spec(*, robust_context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "rlrmp.d55_soft_pgd_nominal_velocity_profiles.v1",
        "issue": ISSUE,
        "figure_kind": "soft_pgd_nominal_forward_velocity_profiles",
        ...
        "transform": [
            {
                ...
                "kwargs": {
                    "row_labels": {row.run_id: row.label for row in SOFT_ROWS},
                    "n_rollout_trials": DEFAULT_N_ROLLOUT_TRIALS,
                    "use_validation_selected_checkpoints": False,
