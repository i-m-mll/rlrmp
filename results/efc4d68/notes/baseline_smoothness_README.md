...
A/B retrain of the existing Part 2.5 baseline (`_artifacts/e81f491/runs/baseline__standard_12k/`) with the compositional `||h_t - h_{t-1}||²` hidden-state smoothness penalty enabled at weight `1e-3`. This is the rlrmp wiring for issue `efc4d68` and feedbax issue `e8f3738` (StateDerivativeLoss).
...
`results/efc4d68/notes/run.json`.
...
The single delta vs. the existing baseline `_artifacts/e81f491/runs/baseline__standard_12k/config.json` is `nn_hidden_derivative: 0.001` (was implicitly 0) and the forced `streaming_loss: false` documented above.
...
`_artifacts/e81f491/runs/baseline__standard_12k/`:
