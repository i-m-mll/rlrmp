...
SPECS = (
    ...
    REPO_ROOT
    / "results/c92ebd8/figures/pgd_1p05_moderate_perturbation_response_overlays/spec.json",
...
def test_all_specs_real_execute_with_data_bound_cardinality(
    ...
    manifest = AnalysisRunManifest(
        ...
        metadata={
            "figure_payload": response_norm_payload(
                _rows(model_count),
                metrics=("position", "velocity"),
                condition_classes=("moderate", "calibrated"),
            )
        },
    )
    ...
    for path in SPECS:
        ...
        output, manifest_path = execute_figure_spec(
            spec.model_copy(update={"inputs": [parent]}), root=tmp_path, issues=["a778c65"]
        )
