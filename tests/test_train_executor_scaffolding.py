...
def test_adaptive_epsilon_resume_transform_accepts_larger_n_train_batches(
    ...
    loaded = load_latest_checkpoint(
        ...
        expected_phase_program=program,
        expected_slots=expected_slots,
        resume_slot_transform=_adaptive_epsilon_resume_slot_transform(None),
        continuation_request=CheckpointContinuationRequest(
            source_completed_batches=2,
            target_total_batches=resumed_n_train_batches,
            batch_indexed_leaves=[
                BatchIndexedCheckpointLeafSpec(slot=OPTIMIZER, tree_path=path)
                for path in (
                    "/gradient/gradient_norm_pre_clip",
                    "/gradient/gradient_clipped",
                    "/gradient/learning_rate",
    ...
    assert optimizer["gradient"].gradient_norm_pre_clip.tolist() == [0.0, 1.0, 2.0, 3.0]
    assert optimizer["update"].update_norm.tolist() == [1.0, 2.0, 3.0, 4.0]
    assert loaded.slots[TRAIN_LOSS] == 0.0
...
def _standard_initial_slots() -> dict[str, object]:
    return {
        ...
        "batch_counter": jnp.asarray(0, dtype=jnp.int32),
    }
...
def _chunked_registry_with_history_sink() -> tuple[TrainingMethodRegistry, object]:
    ...
    def gradient_update(slots, coordinate, context):
        ...
        updates[HISTORY_CHUNK_BYTES] = f"history-{coordinate.program_step}".encode()
        return updates

    def keep_training(slots, coordinate, context):
        del slots
        assert RLRMP_RUNTIME_CONTEXT_KEY in context
        return coordinate.program_step < 2
